...
export default function ControlDock(props: Props) {
  return (
    <footer className="room-dock">
      <button type="button" className="dock-command" onClick={props.onOpenMusic}>
        <Headphones size={15} />
        <span>{props.sourceName ?? "Add music"}</span>
      </button>
      <span className="dock-divider" aria-hidden="true" />
      <button type="button" className="dock-command" onClick={props.onOpenMixer}>
        <SlidersHorizontal size={15} />
        <span>{props.ambientSummary}</span>
      </button>
      <div className="layout-switcher" role="group" aria-label="Display mode">
